const menu = [
  ...
  function displayMenuItems(menu) {
    ...
    menu.forEach((item) => {
      ...
      cardEle.classList.add('col-md-6', 'col-lg-4', 'mb-4');
      cardEle.innerHTML = `
        <div class="card">
      ...
      `;
      menuContainer.appendChild(cardEle);
    });
  }
  ...
  const buttonEle = document.querySelectorAll(".nav-link");

  buttonEle.forEach((button)=>{
    button.addEventListener('click', (e)=>{
      const category = e.target.getAttribute('data-filter');
      buttonEle.forEach(btn=>btn.classList.remove('active'));
  ...
  searchEle.addEventListener('input', (e)=>{
    ...
    const filteredItems = menu.filter((item)=>item.title.toLowerCase().includes(searchTerm));
    displayMenuItems(filteredItems);
  });